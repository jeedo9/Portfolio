...
<template>
         <section id="skills" :class="twMerge('flex justify-start sm:my-0 my-20 items-center flex-col gap-y-34 md:gap-y-37 lg:gap-y-14', $attrs.class as string)">
        <div class="flex justify-center items-center gap-4 flex-col">
          ...
          <p id="sentence" class="mt-5 font-semibold overline decoration-2 decoration-double decoration-muted md:text-lg whitespace-nowrap text-base">{{ sentence }}</p>
        </div>
        <div class="flex justify-center items-start lg:items-center" style="--h-full: 510px; height: var(--h-full);">
          ...
          <ul id="tech" aria-labelledby="dev" aria-describedby="sentence" class="flex justify-center items-center gap-x-4 sm:gap-x-6 gap-y-3 sm:gap-y-5 max-w-full sm:max-w-10/12 xl:max-w-[45%] flex-wrap">
...
<UiBadgesBadgeGradient v-for="stack in skills" :key="stack.name" tabindex="0" :style="{animationDelay: stack.delay}" :class="['skill group focus-visible:outline-0', {'opacity-0': observe}]" >
    ...
    <Icon  id="software-skill" :class="['sm:text-5xl text-4xl',{'rounded-md': stack.name.toLowerCase().includes('typescript')}]" aria-hidden="false" :aria-label="stack.name" :name="stack.iconName" />
