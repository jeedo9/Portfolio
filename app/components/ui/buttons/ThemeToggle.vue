...
<template>
          <button @click="modeValue === 'light' ? useColorMode().preference = 'dark' : useColorMode().preference = 'light'" v-bind="rest"   :type :class="twMerge(` size-7 flex justify-center items-center rounded-md bg-muted/22 hover:bg-muted/40 transition-[background-color]`, $attrs.class as string)">
