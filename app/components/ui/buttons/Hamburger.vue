...
const  {type = 'button', className = '', classNameBars : [bar1 = '', bar2 = '', bar3 = ''] = [], "aria-controls" : ariaControls = 'menu', "aria-label" : ariaLabel = 'Menu button', ...rest} = props

</script>

<template>
    <button @click="injectValues?.toggleMenu" :type="type" :aria-controls="ariaControls" v-bind="rest" :aria-expanded="isMenuOpen ?? 'false'" :aria-label="ariaLabel" :class="twMerge('flex justify-center items-end group flex-col md:hidden w-5 h-1/4 relative', className)"> 
